...
export const getTemperatureColor = (temp: number) => {
  if (temp < 18) return colors.temperature.cold;
  if (temp > 28) return colors.temperature.hot;
  return colors.temperature.normal;
};

export const getHumidityColor = (humidity: number) => {
  if (humidity < 30) return colors.humidity.low;
  if (humidity > 70) return colors.humidity.high;
  return colors.humidity.normal;
};