...
function zoneToTurfFeature(zone: ClientZone): any {
  console.log('Converting zone to turf feature:', zone.type, zone.geometry?.type);
...
function createAnalysis(feature: any, operation: string, _zoneCount: number) {
  ...
  const analysis = {
    ...
    riskLevel: (area > 10 ? 'high' : area > 2 ? 'moderate' : 'low') as 'low' | 'moderate' | 'high' | 'critical',
    ...
    switch (operation) {
      ...
      case 'intersection':
        ...
        try {
          ...
          const featureCollection = turf.featureCollection(intersectionFeatures);
          console.log('About to call turf.intersect with FeatureCollection:', featureCollection);
      ...
      case 'contains':
        ...
        return {
          ...
          environmentalAnalysis: {
            ...
            affectedPopulation: Math.round(turf.area(container) / 1000000 * 500),
...
export function analyzePopulationImpact(
  ...
  populationCenters: Array<{lat: number, lng: number, population: number, name: string}>
): {
    ...
    for (const center of populationCenters) {
      ...
      if (turf.booleanPointInPolygon(point, turfZone)) {
        ...
        affectedCenters.push({
          ...
          risk: center.population > 10000 ? 'high' : center.population > 1000 ? 'moderate' : 'low'
        });
      }
    }
  }
  ...
  if (totalAffected > 50000) {
    recommendations.push('🚨 Large population at risk - immediate action required');
