...
import { performGeometryOperation } from '@/lib/geometryUtils'
...
export default function MapPage() {
  ...
  const { sensorUpdateTrigger, shapeUpdateTrigger, triggerShapeUpdate } = useAppUpdates()
  const { 
    isHistoricalMode, 
    selectedDateTime, 
    historicalShapes, 
  ...
  useEffect(() => {
    if (isAuthenticated && sensorUpdateTrigger > 0) {
      console.log('🔄 Sensor change detected, updating shape pollution data...')
  ...
  useEffect(() => {
    if (savedShapes.length > 0) {
      console.log('🎨 Shape data changed, triggering redraw:', savedShapes.length, 'shapes')
  ...
  const loadSavedShapes = async () => {
    try {
      ...
      const endpoint = isHistoricalMode ? '/shapes/historical' : '/shapes/geometry'
      const params = isHistoricalMode ? { timestamp: selectedDateTime.toISOString() } : {}
      
      const response = await api.get(endpoint, { params })
      
      if (response.data.success && response.data.data) {
        console.log('📊 Loading shapes with pollution data:', response.data.data.shapes.length)
        
        // Process shapes to ensure proper GeoJSON format
        const processedShapes = response.data.data.shapes.map((shape: any) => {
          try {
      ...
      try {
        ...
        if (fallbackResponse.data.success && fallbackResponse.data.data) {
          console.log('Fallback: Loaded shapes without geometry:', fallbackResponse.data.data.shapes)
  ...
  const handleSaveShapes = async () => {
    ...
    try {
      ...
      for (const shape of temporaryShapes) {
        const shapePayload = {
          name: shape.properties?.name || `Shape ${new Date().toLocaleTimeString()}`,
  ...
  const handleClientZoneSelect = (zoneId: string) => {
    if (zoneId === 'clear-all') {
      setSelectedZones([])
      setClientZones(prev => prev.map(zone => ({ ...zone, selected: false })))
      return
    }
    ...
    setSelectedZones(prev => {
      ...
      setClientZones(prevZones => prevZones.map(zone => ({
        ...zone,
  ...
  const handleGeometryOperation = (operation: string, zoneIds: string[]) => {
    const selectedZoneObjects = clientZones.filter(zone => zoneIds.includes(zone.id))
    
    if (selectedZoneObjects.length < 2) return
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-foreground/60">
                Welcome, {user?.name} • {isHistoricalMode 
            ...
            <div className="flex items-center gap-3">
              {temporaryShapes.length > 0 && (
                <Button
                  ...
                  isDisabled={saving}
                >
                  {saving ? 'Saving...' : `Save ${temporaryShapes.length} Shape${temporaryShapes.length > 1 ? 's' : ''}`}
                </Button>
              )}
              <Button
                variant="bordered"
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="w-80 bg-content1 border-r border-divider p-4 overflow-y-auto">
            ...
            <PanelSwitcher
              ...
              onPlacementModeChange={isHistoricalMode ? () => {} : setIsPlacementMode}
              clientZones={isHistoricalMode ? [] : clientZones}
              selectedZones={isHistoricalMode ? [] : selectedZones}
              onZoneSelect={isHistoricalMode ? () => {} : handleClientZoneSelect}
              onZoneDelete={isHistoricalMode ? () => {} : handleClientZoneDelete}
              onClearAllZones={isHistoricalMode ? () => {} : handleClearAllClientZones}
              onGeometryOperation={isHistoricalMode ? () => {} : handleGeometryOperation}
              operationResults={isHistoricalMode ? [] : operationResults}
              onClearResults={isHistoricalMode ? () => {} : handleClearOperationResults}
            />
              ...
              <div className="mt-6 p-4 bg-content2 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-success">{displayShapes.length}</span>
                  </div>
                  {!isHistoricalMode && (
                    <div className="flex justify-between">
                      <span className="text-foreground/70">Temporary Shapes:</span>
                      <span className="font-medium text-warning">{temporaryShapes.length}</span>
                    </div>
                  )}
              ...
              <div className="mt-6 p-4 bg-content2 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-foreground/70">
                    {historicalLoading ? 'Loading historical data...' : 'Loading saved shapes...'}
                  </span>
                </div>
              </div>
            )}
            ...
            {historicalError && (
              <div className="mt-6 p-4 bg-danger/10 border border-danger/20 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-danger">Historical Data Error</p>
          ...
          <div className="flex-1 relative">
            <Map
              ...
              clientZones={isHistoricalMode ? [] : [...clientZones, ...operationResults]}
              onClientZoneCreated={isHistoricalMode ? () => {} : handleClientZoneCreated}
              onClientZoneClick={isHistoricalMode ? () => {} : handleClientZoneClick}
            />
          </div>
        </div>
      </div>
        ...
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">
            ...
            <p className="text-sm text-foreground/60">
              Give your {pendingShape?.properties?.type || 'shape'} a descriptive name
            </p>
          </ModalHeader>
          <ModalBody>
            <Input
