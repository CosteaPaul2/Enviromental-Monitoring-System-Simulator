...
export function SensorReadingCard({
  ...
  return (
    ...
    >
      <Card
        className={`w-full ${compact ? "h-24" : "h-32"} ${!sensor.active ? "opacity-60" : ""}`}
      >
          ...
          <div className="flex items-center justify-between h-full">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {!compact && (
                  <p className="text-xs text-default-400 mt-1">{timeAgo}</p>
                )}
              </div>
            </div>
            ...
            <div className="text-right flex-shrink-0">
              {reading ? (
                <>
                  <div
                    className={`font-bold ${compact ? "text-lg" : "text-2xl"} mb-1`}
                  >
                    {reading.value.toFixed(1)}
                  </div>
                  <div className="flex items-center gap-1 justify-end">
                    <span
                      className={`text-xs ${compact ? "text-xs" : "text-sm"} text-default-500`}
                    >
                      {typeInfo.unit}
                    </span>
                    <Chip
                ...
                <div className="text-center">
                  <Icon
                    className="text-warning text-xl mb-1"
                    icon="tabler:alert-triangle"
                  />
