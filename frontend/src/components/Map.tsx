  ...
  useEffect(() => {
    ...
    function setupPolygonDrawing() {
      ...
      const onMapClick = (e: L.LeafletMouseEvent) => {
        ...
        if (points.length === 1) {
          console.log(
            "🔷 Polygon started! Click more points, then double-click to finish",
          );
        } else if (points.length >= 3) {
          console.log(
            `🔷 ${points.length} points added. Double-click to finish polygon.`,
          );
        }
      };
      ...
      const onMapDoubleClick = (e: L.LeafletMouseEvent) => {
        ...
        if (points.length < 3) {
          console.log(
            "Need at least 3 points for polygon. Current points:",
            points.length,
          );
      ...
      console.log(
        "Polygon drawing mode active. Click to add points, double-click to finish.",
      );
    }
      ...
      try {
        ...
        if (isCircle) {
          ...
          if (geometry.type === "Point") {
            ...
            console.warn(
              "Found circle stored as polygon - fixing legacy data:",
              shape.name || geoJsonShape.properties?.name || `ID: ${shape.id}`,
            );
            ...
            console.log(
              `Legacy circle converted: center [${centerLat}, ${centerLng}], radius ${radius}m`,
            );
          }
        } else {
          const pollutionColor = shape.pollutionLevel
          ...
          const addClickHandler = (leafletLayer: any) => {
            if (
              ...
              leafletLayer.on("click", (e: L.LeafletMouseEvent) => {
                ...
                if (selectedSensor) {
                  console.log(
                    "🚫 Shape click prevented - sensor placement mode active:",
                ...
                console.log(
                  "✅ Shape clicked:",
                  shape.id || geoJsonShape.properties?.id,
                );
    ...
    const onMouseUp = (e: L.LeafletMouseEvent) => {
      ...
      if (circle && onClientZoneCreated) {
        ...
        const clientZone: ClientZone = {
          ...
          color: "#3b82f6",
          created: new Date(),
          radius: radius, // Store radius at zone level
        };
    ...
    clientZones.forEach((zone) => {
      try {
        ...
        if (zone.type === "rectangle") {
          ...
          // Check if this is an operation result (has environmentalAnalysis)
          ...
          if (zone.geometry.type === "Polygon") {
            ...
            // Handle MultiPolygon by rendering multiple polygons
...
function SensorPlacementControls({
  selectedSensor,
  onSensorPlaced,
